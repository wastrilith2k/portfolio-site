...
import { trackPageView, portfolioAnalytics } from '../services/analytics.service'
...
export const usePortfolioAnalytics = () => {
  return {
    ...
    trackChatMessage: useCallback((messageType: 'user_message' | 'bot_response') => {
    ...
    trackContactForm: useCallback((action: 'started' | 'completed' | 'abandoned') => {
...
export const useSectionTracking = (sectionName: string, threshold = 0.5) => {
  ...
  const trackSection = useCallback((node: HTMLElement | null) => {
    if (!node) return
